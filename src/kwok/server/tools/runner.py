...
class ToolRunner:
    ...
    async def _run_with_governance(
        ...
        while True:
            ...
            try:
                return await self._run_once(tool, args, per_timeout)
            except ToolError as exc:
                if not self._can_retry(retry_policy, exc, attempt):
                    return self._tool_error_text(tool, exc)
            except TimeoutError:
                if not self._can_retry(retry_policy, TimeoutError(), attempt):
                    return "工具执行超时"
            except Exception as exc:
                if not self._can_retry(retry_policy, exc, attempt):
                    return f"工具执行失败：{exc}"
            # 到达此处说明需要重试：退避后进入下一轮（deadline 在循环顶部再校验）
            await self._backoff(retry_policy, attempt, deadline)
            attempt += 1
